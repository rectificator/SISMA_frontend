<template>
    <div>
        <div class="flex bg-black text-white text-lg">
            <p class="py-2">{{ formName }}</p>
        </div>
        <form
...
export default {
    ...
    methods: {
        ...
        addFieldToError(field) {
            ...
            this.$set(this.errors, field, field in this.info.info_registro)
        },
        async submitForm() {
            this.validateForm()
        ...
        getPoblacion(value) {
            //console.log('INFO: ', this.info)
            this.info[this.formRoute].poblacion = value
        },

        getModalidad(modalidad) {
            //console.log('INFO: ', this.info)
            this.info[this.formRoute].modalidad = modalidad
        },
