    ...
    <div>
        ...
        >
            ...
            <px-checkbox-group-mes
                ...
                v-model="mesObjects"
            ></px-checkbox-group-mes>
            <span v-show="false">{{ getMeses(mesObjects) }}</span>
    ...
    methods: {
        ...
        addFieldToError(field) {
            ...
            this.$set(
                this.errors,
                field,
                this.fieldIsInObject(field, this.info.info_registro) ||
                    this.fieldIsInObject(field, this.info[this.formRoute])
            )
        },
        fieldIsInObject(field, object) {
            return field in object
        },
        fieldInObjectIsNotEmpty(field, object) {
            let fieldIsNotUndefined = this.info[object][field] != undefined
            let fieldIsNotEmptyArray = this.info[object][field] != []
            let fieldIsNotEmptyString = this.info[object][field] != ''
            let fieldIsNotNull = this.info[object][field] != null
            return (
                fieldIsNotUndefined &&
        ...
        getAnio(anio) {
            ...
            this.info.info_registro = this.addValueToFieldToObject(
                anio.id,
                'id_anio',
                this.info.info_registro
            )
            this.log('ANIO INFOREG: ', this.info.info_registro)
        },

        getMeses(meses) {
            //console.log('INFO: ', this.info)
            this.info.info_registro.id_mes = meses.map((mes) => {
                return mes.id
            })
        ...
        addValueToFieldToObject(value, field, object) {
            if (this.fieldInObjectIsNotEmpty(field, object)) {
                object[field] = value
            }
            return object
        },
