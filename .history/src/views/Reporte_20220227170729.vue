...
export default {
    ...
    computed: {
        capacitacionSaludMentalEstatalDataSet() {
            ...
            capacitacionAux.forEach((capacitacion) => {
                ignoreTagList.forEach((key) => {
                    delete capacitacion[key]
                })
            })

            let capKeys = Object.keys(capacitacionAux)
            console.log('CapacitacionAUX: ', capacitacionAux)
            for (let keyIndex = 0; keyIndex < capKeys.length; keyIndex++) {
                let key = capKeys[keyIndex]
