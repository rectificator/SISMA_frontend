<template>
    <div>
        ...
        >
            <px-select-anio
                :error="'anio' in errors"
                @select-value="getAnio"
            ></px-select-anio>
            <px-select-mes
                :error="'mes' in info.info_registro"
                @select-value="getMes"
            ></px-select-mes>
            <px-auto-complete-estado
                :error="'estado' in info.info_registro"
                @select-value="getEstado"
            />
            <px-auto-complete-municipio
                :error="'municipio' in info.info_registro"
                @select-value="getMunicipio"
            />
            <px-auto-complete-institucion
                :error="'institucion' in info.info_registro"
    ...
    methods: {
        validateForm() {
            ...
            for (let key in this.errors) {
                if (this.errors[key] == false) {
                    console.log('ValidateErrors: ', this.validate)
                    break
                } else {
                    this.validate = true
                }
            ...
            if (
                this.info[this.formName].keys.length == 0 ||
                this.info.info_registro.keys.length == 0
            ) {
                this.validate = false
            }

            console.log(`${this.formName}: `, this.info[this.formName])
            console.log('INFO_REGISTRO: ', this.info.info_registro)
            console.log('ValidateErrors2: ', this.validate)
        },
        addToError() {
        ...
        async submitForm() {
            ...
            const res = this.validate
                ? await fetch(`${this.apiBaseUrl}acciones_comunitarias`, {
                      method: 'POST',
                      mode: 'cors',
                      headers: {
                ...
                : await (async () => 'Aun hay campos sin información')
            let t = typeof res
            if (t == 'String') {
                console.log('RES: ', res)
            } else {
        ...
        getAccionesComunitarias(data) {
            ...
            this.errors =
                { ...this.errors, ...acciones_comunitarias.errors } /
                delete acciones_comunitarias.errors
