    ...
    computed: {
        ...
        capacitacionSaludMentalPorEstado() {
            ...
            if (capacitacionAux != undefined && capacitacionAux.length > 0) {
                ...
                deletePropertiesFromObject(ignoreTagList, capacitacionAux)
        ...
        personasAlcanzadasCapacitacion() {},
    ...
    methods: {
        getAndAddPropertieValuesThatIsNotInTheTagListFromFormObjectForEveryState(
            ...
            if (formObjectList != undefined && formObjectList.length > 0) {
                ...
                this.forEveryStateExecute((state, stateIndex) => {
                    for (let formItem of auxList) {
                        for (const key in formItem) {
                            if (key != 'info_registro') {
                                let municipio = this.municipalityOf(formItem)
                                if (municipio.id_estado == state.id) {
                                    dataArray[stateIndex] += formItem[key]
                                }
                            }
                        }
                    }
