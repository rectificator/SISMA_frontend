...
export default {
    ...
    computed: {
        ...
        consultasDataCollection() {
            ...
            if (dataValues[0] == 0) {
                dataValues = new Array(7).map(() => Math.random())
            }
            return {
                labels: [
                    'DEPRESION',
        ...
        valoresParaLaDistribucionPorDiagnostico() {
            let values = []
            console.log('VAL_DIAG: ', this.valoresPorDiagnostico)
            for (let dignostico of this.valoresPorDiagnostico) {
                values.push(this.calculateTotalFromArray(dignostico))
                console.log('COL_VAL: ', values)
            }
            return values
        },
    },
