...
export default {
    ...
    computed: {
        ...
        conveniosDeColaboracion() {
            return this.getPropertieFromFormObjectByEveryState(
                'convenios',
                this.politicas_publicas.convenios
            )
        },
        beneficiosOtorgados() {
            return this.getPropertieFromFormObjectByEveryState(
                'beneficios_otorgados',
                this.politicas_publicas.beneficios_otorgados
            )
        },
        politicasPublicas() {
            return this.getPropertieFromFormObjectByEveryState(
    ...
    methods: {
        ...
        getInfo(token) {
            ...
            api.getFrom('acciones_prevencion_salud', token).then((data) => {
                //console.log('DATA: ', data)
                this.acciones_prevencion_salud = data
            })
            api.getFrom('acciones_comunitarias', token).then((data) => {
                //console.log('DATA: ', data)
                this.acciones_comunitarias = data
            })
            api.getFrom('atencion_salud', token).then((data) => {
                //console.log('DATA: ', data)
                this.atencion_salud = data
            })
            api.getFrom('capacitacion', token).then((data) => {
                //console.log('DATA: ', data)
                this.capacitacion = data
            })
            api.getFrom('politicas_publicas', token).then((data) => {
                //console.log('DATA: ', data)
                this.politicas_publicas = data
            })
            api.getFrom('suicidio', token).then((data) => {
                //console.log('DATA: ', data)
                this.suicidio = data
            })
        },
    ...
    created() {
        //console.log('JWT 2', this.jwt)
        this.getInfo(this.jwt)
    },
    watch: {},
}
</script>
