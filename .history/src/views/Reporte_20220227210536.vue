        ...
        >
            ...
            <div class="block flex justify-end bg-transparent">
                <button
                    :class="
                        reportSelect == false
                            ? 'rounded-md bg-green-700 hover:text-xl hover:bg-green-500 focus:bg-green-500 focus:text-white focus:outline-none focus:shadow-outline'
                            : 'rounded-md bg-gray-700 hover:text-xl hover:bg-green-500 focus:bg-green-500 focus:text-white focus:outline-none focus:shadow-outline'
                    "
                ...
                <button
                    :class="
                        reportSelect == true
                            ? 'rounded-md bg-blue-700 hover:text-xl hover:bg-blue-500 focus:bg-blue-500 focus:text-white focus:outline-none focus:shadow-outline'
                            : 'rounded-md bg-gray-700 hover:text-xl hover:bg-blue-500 focus:bg-blue-500 focus:text-white focus:outline-none focus:shadow-outline'
                    "
...
export default {
    ...
    computed: {
        capacitacionSaludMentalEstatalDataSet() {
            ...
            let capacitacionAux = this.capacitacion
                    ...
                    for (let capacitacion of capacitacionAux) {
                        for (const key in capacitacion) {
                            if (key != 'info_registro') {
                                let municipio = this.municipalities.find(
                                    (municipality) => {
                                        return (
                                            municipality.municipio ==
                                            capacitacion.info_registro.municipio
                                        )
                                    }
                                )
